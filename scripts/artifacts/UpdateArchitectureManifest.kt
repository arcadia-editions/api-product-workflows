...
private fun validateRequestedArtifacts(service: ManifestService, family: String) {
    if (family == "api-product") {
        listOf("zdl", "openapi", "asyncapi").forEach { type ->
            ...
            check(entries.size == 1) { "expected exactly one $type artifact before service release" }
            check(!entries.single().version.isNullOrBlank()) {
                "$type must have an explicit audited version before a service release"
            }
        }
        ...
        if (clients.size == 1) {
            check(!clients.single().version.isNullOrBlank()) {
                "asyncapi-client must have an explicit audited version before a service release"
            }
        }
        return
    }
